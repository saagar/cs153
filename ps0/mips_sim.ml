...
let execute_beq rs rt offset current_state : state =
  let rs32 = rf_lookup (reg2ind rs) current_state.r in
  let rt32 = rf_lookup (reg2ind rt) current_state.r in
  let amount_to_advance_pc = if (Int32.to_int rs32 = Int32.to_int rt32) then offset else Int32.of_int 4 in
...
let execute_lw (rt : reg) (rs : reg) (offset : int32) (current_state : state) : state =
  ...
  let byte2 = mem_lookup (Int32.add target_address (Int32.of_int 1)) current_state.m in
  let byte3 = mem_lookup (Int32.add target_address (Int32.of_int 2)) current_state.m in
  let byte4 = mem_lookup (Int32.add target_address (Int32.of_int 3)) current_state.m in
...
let execute_sw (rt : reg) (rs : reg) (offset : int32) (current_state : state) : state =
  ...
  let new_mem2 = mem_update (Int32.add target_address (Int32.of_int 1)) byte2 new_mem in
  let new_mem3 = mem_update (Int32.add target_address (Int32.of_int 2)) byte3 new_mem in
  let new_mem4 = mem_update (Int32.add target_address (Int32.of_int 3)) byte4 new_mem in
  { r = current_state.r; pc = Int32.add current_state.pc (Int32.of_int 4); m = new_mem4 }

let interp_step (current_state : state) : state =
  let first_inst_byte = mem_lookup current_state.pc current_state.m in
  let second_inst_byte = mem_lookup (Int32.add current_state.pc (Int32.of_int 1)) current_state.m in
  let third_inst_byte = mem_lookup (Int32.add current_state.pc (Int32.of_int 2)) current_state.m in
  let fourth_inst_byte = mem_lookup (Int32.add current_state.pc (Int32.of_int 3)) current_state.m in
...
let rec interp (init_state : state) : state =
  ...
  let second_inst_byte = mem_lookup (Int32.add init_state.pc (Int32.of_int 1)) init_state.m in
  let third_inst_byte = mem_lookup (Int32.add init_state.pc (Int32.of_int 2)) init_state.m in
  let fourth_inst_byte = mem_lookup (Int32.add init_state.pc (Int32.of_int 3)) init_state.m in
  let inst32 = int32_from_bytes first_inst_byte second_inst_byte third_inst_byte fourth_inst_byte in
