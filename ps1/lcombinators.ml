...
struct
  ...
  (* the parser [satisfy_opt pred] reads a token [h] from the input list --
   * if [pred h] returns some value, then the parser successeds, consuming
   * and returning the value in [pred h]. Otherwise the parser fails. *)
  let satisfy_opt (pred:'c -> 'a option) : ('c,'a) parser =
      fun cs -> match cs with 
                  | [] -> Nil
                  | h::t -> (match pred h with
                    | Some v -> singleton (v,t)
                    | None -> Nil)
  ...
  let lazy_seq((p1:('c,'a) parser Lazy.t), (p2:('c,'b) parser Lazy.t)) : ('c,'a * 'b) parser = 
      fun cs -> 
        lflatten (lmap (fun (v1,cs1) -> 
                          lmap (fun (v2,cs2) -> 
                                  ((v1,v2),cs2)) ((Lazy.force p2) cs1)) ((Lazy.force p1) cs))
  ...
  (* creates a parser that returns the value [v] for parser [p] *)
  let rec const_map (v : 'c) (p : ('a, 'b) parser) : ('a, 'c) parser = 
    map (fun _ -> v) p 
...
struct
  ...
  (* [comment] matches C-style comments that begin with /* and end with */. *)
